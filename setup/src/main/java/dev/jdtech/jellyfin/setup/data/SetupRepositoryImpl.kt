...
import dev.jdtech.jellyfin.curated.api.HealthDto
...
class SetupRepositoryImpl(
    ...
    override suspend fun addServer(address: String): Server {
        ...
        val normalizedBaseUrl =
            try {
                CuratedUrlResolver.normalizeBaseUrl(address)
        ...
        if (normalizedBaseUrl != null) {
            val health =
                try {
                    ...
                    Timber.e(e, "Address did not respond as a Curated server")
                    throw ExceptionUiText(curatedConnectionFailureUiText(normalizedBaseUrl, e))
                }
            return saveCuratedServerInDatabase(normalizedBaseUrl, health)
        }
    ...
    override suspend fun getCuratedAuthStatus(): AuthStatusDto =
        withContext(Dispatchers.IO) { currentCuratedApi().getAuthStatus() }

    override suspend fun unlockCurated(pin: String, trustedForever: Boolean): AuthStatusDto =
        withContext(Dispatchers.IO) {
            currentCuratedApi().unlock(pin = pin, trustedForever = trustedForever)
        }

    private fun currentCuratedApi(): CuratedApiClient =
        CuratedApiClient(
            baseUrl = currentServerAddress(),
            client = curatedHttpClient,
        )

    private fun currentServerAddress(): String {
        val serverId =
            appPreferences.getValue(appPreferences.currentServer)
                ?: throw ExceptionUiText(
                    UiText.StringResource(SetupR.string.add_server_error_not_found)
                )
        return database.getServerWithAddressAndUser(serverId)?.address?.address
            ?: throw ExceptionUiText(UiText.StringResource(SetupR.string.add_server_error_not_found))
    }

    private fun saveCuratedServerInDatabase(
        normalizedBaseUrl: String,
        health: HealthDto,
    ): Server {
        val serverId = curatedServerIdForBaseUrl(normalizedBaseUrl)
        val serverInDatabase = database.get(serverId)

        val server =
            if (serverInDatabase != null) {
                val addresses = database.getServerWithAddresses(serverInDatabase.id).addresses
                if (addresses.none { it.address == normalizedBaseUrl }) {
                    val serverAddress =
                        ServerAddress(
                            id = UUID.randomUUID(),
                ...
                val server =
                    Server(
                        id = serverId,
                        name = health.name.ifBlank { "Curated" },
        ...
        jellyfinApi.apply {
            api.update(baseUrl = normalizedBaseUrl, accessToken = null)
            userId = null
        }
...
internal fun curatedServerIdForBaseUrl(baseUrl: String): String {
    val normalizedBaseUrl = CuratedUrlResolver.normalizeBaseUrl(baseUrl)
    return UUID.nameUUIDFromBytes(normalizedBaseUrl.toByteArray(StandardCharsets.UTF_8)).toString()
}

internal fun curatedConnectionFailureUiText(
    normalizedBaseUrl: String,
    cause: Exception,
): UiText {
    val healthUrl = CuratedUrlResolver.apiUrl(normalizedBaseUrl, "/health")
    val reason = cause.localizedMessage?.takeIf { it.isNotBlank() } ?: cause.javaClass.simpleName
    return UiText.DynamicString("Curated server connection failed: $healthUrl ($reason)")
}
