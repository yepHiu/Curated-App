...
import dev.curated.app.core.presentation.dummy.dummyHomeSection
import dev.curated.app.core.presentation.dummy.dummyHomeSuggestions
import dev.curated.app.core.presentation.dummy.dummyHomeView
import dev.curated.app.core.presentation.dummy.dummyServer
import dev.curated.app.film.presentation.home.HomeAction
import dev.curated.app.film.presentation.home.HomeState
import dev.curated.app.film.presentation.home.HomeViewModel
...
import dev.curated.app.presentation.film.components.HomeCarousel
import dev.curated.app.presentation.film.components.HomeHeader
import dev.curated.app.presentation.film.components.HomeSection
import dev.curated.app.presentation.film.components.HomeView
import dev.curated.app.presentation.film.components.ServerSelectionBottomSheet
...
private fun HomeScreenLayout(state: HomeState, onAction: (HomeAction) -> Unit) {
    ...
    val rawSuggestionsSection = state.suggestionsSection
    val suggestionsSection =
        rawSuggestionsSection
            ?.copy(items = rawSuggestionsSection.items.filter(::isCuratedVisibleItem))
    ...
    val resumeSection =
        state.resumeSection
            ?.let { section ->
                section.copy(
                    homeSection =
                        section.homeSection.copy(
                            items = section.homeSection.items.filter(::isCuratedVisibleItem)
                        )
                )
            }
            ?.takeIf { it.homeSection.items.isNotEmpty() }
    val nextUpSection =
        state.nextUpSection
            ?.let { section ->
                section.copy(
            ...
            ?.takeIf { it.homeSection.items.isNotEmpty() }
    val views = state.views.filter { isCuratedVisibleCollectionType(it.view.type) }
