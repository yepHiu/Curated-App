package dev.curated.app.presentation.setup.addserver
...
import dev.curated.app.presentation.setup.components.DiscoveredServerItem
...
import dev.curated.app.setup.presentation.addserver.AddServerAction
import dev.curated.app.setup.presentation.addserver.AddServerEvent
import dev.curated.app.setup.presentation.addserver.AddServerState
import dev.curated.app.setup.presentation.addserver.AddServerViewModel
import dev.curated.app.utils.ObserveAsEvents

@Composable
fun AddServerScreen(
    onSuccess: () -> Unit,
    onAuthLockRequired: () -> Unit,
    onBackClick: () -> Unit,
    ...
    ObserveAsEvents(viewModel.events) { event ->
        when (event) {
            is AddServerEvent.NavigateHome -> onSuccess()
            is AddServerEvent.NavigateAuthLock -> onAuthLockRequired()
        }
    }
...
private fun AddServerScreenLayout(state: AddServerState, onAction: (AddServerAction) -> Unit) {
    ...
    val doConnect = { onAction(AddServerAction.OnConnectClick(state.serverAddress)) }
    ...
    RootLayout {
        Column(
            ...
            OutlinedTextField(
                ...
                onValueChange = { onAction(AddServerAction.OnAddressChange(it)) },
                label = {
...
private fun AddServerScreenLayoutPreview() {
    CuratedTheme { AddServerScreenLayout(state = AddServerState(), onAction = {}) }
}
