...
- Android 隐私防护由 App 自己注册的 `Application.ActivityLifecycleCallbacks` 驱动，不依赖 `ProcessLifecycleOwner`；默认开启系统媒体静音、播放器内部音量归零、50% 黑色视觉遮挡、Android 12+ blur，以及 `FLAG_SECURE` 截图/录屏/最近任务预览保护。
